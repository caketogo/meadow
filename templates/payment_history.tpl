		...
		<div class="col-md-12" id="content_secion">
				...
				<div class="box box-primary" id="scrollable_content1">
					...
					<div class="row" >
							<div class="col-md-12 ">
								...
								<div class="col-md-4">
									<h2 class="box-title">{*$Customer_full_name*} Assigned Container (s)</h2>
								</div>
								<div class="col-md-8" style="margin-top:10px">
									<a class="col-xs-12 col-sm-4 col-md-4 btn bg-primary btn-primary margin" href="{$siteadmin_globalpath}assign_container.php?customer_id={$current_customer_id}" title="Create Manual Container">
									Assign Container
									...
									<!--<a class="new_admin_class" href="{$siteadmin_globalpath}payment_process.php?customer_id={$current_customer_id}" id="process_payment" title="De assign"><p style="font-size:28px;color:green;">Process a Payment</p></a>-->
									...
									<!--{$siteadmin_globalpath}payment_process.php?customer_id={$current_customer_id}-->
									</div>
				...
				<div class="box-body table-responsive">
					...
					<table id="openTrigger_history" class="table table-striped table-bordered scroll">
                        ...
                        {section name="payment" loop=$PaymentHistoryList}
                          ...
                          <tr>
                            <td width="1%"  class="text-center"><input type="checkbox" class="case" name="select_container_number[]" id="select_{$PaymentHistoryList[payment].container_number}" value="{$PaymentHistoryList[payment].container_number}" />  </td>
							
                            <td width="6%">{$PaymentHistoryList[payment].container_type}</td>
							<td width="6%">{$PaymentHistoryList[payment].containerName} <input type="hidden" class="case" name="select_container_number[]" id="select_container_number{$PaymentHistoryList[payment].container_number}" value="{$PaymentHistoryList[payment].container_number}" /></td>
							<td width="7%">{$PaymentHistoryList[payment].contract_start_date}</td>
							<td width="7%">{$PaymentHistoryList[payment].contract_next_due_date}</td>
							<td width="16%">{$PaymentHistoryList[payment].Related_priod_Result}</td>
							<td width="5%">{$PaymentHistoryList[payment].overdue}</td>
							<td width="6%">&pound; {$PaymentHistoryList[payment].licence_amount_vat}</td>
                            <td width="5%">&pound; {$PaymentHistoryList[payment].deposit_amount}</td>
							<td width="8%">&pound; {$PaymentHistoryList[payment].miscellaneous}</td>
							<td width="6%">{$PaymentHistoryList[payment].penality}</td>
							<td width="8%"><p style="margin-left:10px;">&pound; {$PaymentHistoryList[payment].totalAmountDueColumn}</p></td>
							<td width="12%">{*$PaymentHistoryList[payment].status*}
							<div data-toggle="buttons" class="btn-group btn-group-new">
									<input class="btn btn-primary {if $PaymentHistoryList[payment].status eq 'Locked'} active {/if}"  type="button" onclick="updateContainerStatus('{$PaymentHistoryList[payment].container_number}', '{$current_customer_id}', 'Locked')" id="Locked" name="container_status_locked" value="Locked"> 
									<input class="btn btn-primary {if $PaymentHistoryList[payment].status eq 'Occupied'} active {/if}" type="button" onclick="updateContainerStatus('{$PaymentHistoryList[payment].container_number}', '{$current_customer_id}', 'Occupied')"  id="Occupied" name="container_status_occupied" value="Occupied"> 
							</div>
			...
			<div class="box box-primary" id="scrollable_content_second" >
					...
					<div class="row" >
							<div class="col-md-12 ">
								<div class="col-md-6 ">
									<h2 class="box-title">{*$Customer_full_name*} Previous Payment History</h2>
								</div>
		...
		<form >
				...
				<div class="box-body table-responsive" id="my_previous_payment_content">
					...
					<input type="hidden" name="customer_id" id="customer_id" value="{$current_customer_id}" />
						...
						<!--<tbody>
                        {section name="payment" loop=$PreviousPaymentHistory}

                          <tr>
                            <td width="5%">{$PreviousPaymentHistory[payment].container_type}</td>
							<td width="5%">{$PreviousPaymentHistory[payment].containerName}</td>
							<td width="7%">{$PreviousPaymentHistory[payment].due_date}</td>
							<td width="7%">{$PreviousPaymentHistory[payment].related_period}</td>
							<td width="20%">{$PreviousPaymentHistory[payment].date_paid}</td>
							<td width="5%">{$PreviousPaymentHistory[payment].license}</td>
							<td width="5%">&pound; {$PreviousPaymentHistory[payment].deposit}</td>
                            <td width="5%">&pound; {$PreviousPaymentHistory[payment].miscellaneous}</td>
							<td width="7%">&pound; {$PreviousPaymentHistory[payment].penality}</td>
							<td width="7%">{$PreviousPaymentHistory[payment].payment_type}</td>
							<td width="7%">&pound; {$PreviousPaymentHistory[payment].total_amount_due}</td>
