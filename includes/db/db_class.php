<?PHP
	/**
	* Wensel Gallery - Natures Best in Pictures.
	* 
	* PHP version 5
	*
	...
	* This file contains the commonly used Includes, Functions to be used for Wensel Gallery
	* @project Wensel Gallery
	* @version 1.0
	* @copyright  	2006-2010 Securenext Softwares Pvt Ltd
	*	
	* @package Wensel Gallery
	* @subpackage Database
	* @filesource
	*/
		...
		*/		
		...
		function dbConnect() {
			...
			if ($this->dbConnected==0) {
				$this->dbLink = mysqli_connect($this->dbServer, $this->dbUser, $this->dbPass,$this->dbDatabase) or $this->reportError("Could not connect to Server.");
		...
		function dbRunQuery() {
			...
			if ($this->dbConnected == 1) {
				if ($this->dbQuerySet == 1) {			
					...
					//print " SQL ".$this->dbQuery." <br>----------------------------------------------------<br> ";
					switch ($this->dbType) {
					case 1:
						//$this->dbQueryResult = mysqli_query ($this->dbQuery, $this->dbLink) or $this->reportError();
		...
		function reportError($error = "") { // Sends Error Report 1. To logfile 2. To Email 3. system logs
			...
			print mysqli_error($this->dbLink)."<br>CHECK THE URL => <br>".$this->dbQuery;
			switch ($global_config["error_report"]) {
				case 1:	// reportError in logfile			
						echo $this->dbQuery."<br>".mysqli_error($this->dbLink);
