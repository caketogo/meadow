<?php
	class siteadminnew extends commonnew	{
		/* Constructor */
		function siteadmin()	{
			$this->Common();
		...
		function GetContainersDetails() {
			global $global_config, $objSmarty;
				...
				//if($container_rental_user_type == 'super_admin'){
...
print_r($get_userdet_res);
exit;*/
			$existed = $get_userdet_res[0]['address_proof'];
			$existed_id = $get_userdet_res[0]['id_proof'];
			
			$explode = explode('uploads',$existed);
			$explode1 = explode('/',$existed);
			$count_explode1 = count($explode1);
			$need = $explode1[$count_explode1-1];
			$formed = $global_config["site_globalpath"].'uploads'.$explode[1];

			$get_userdet_res[0]['dynamic_link'] = $formed;
			$get_userdet_res[0]['uploaded_file_name'] = $need;

			$explode_iss = explode('uploads',$existed_id);
			$explode2 = explode('/',$explode_iss);
			$count_explode2 = count($explode2);
			$need1 = $explode2[$count_explode2-1];
			$formedchk = $global_config["site_globalpath"].'uploads'.$explode_iss[1];
			
			$get_userdet_res[0]['dynamic_idproof'] = $formedchk;
		...
		function CustomerAuthordetails($customerID) {
			global $global_config, $objSmarty;
			$get_userdet_qry = "SELECT * FROM tbl_authorised_users where customer_id = '".$customerID."' order by id";
			$get_userdet_res = $this->SelectQryDirect($get_userdet_qry);
			//$get_userdet_qry = "SELECT * FROM tbl_customer where id = '".$customerID."'";
			//$get_userdet_res = $this->SelectQryDirect($get_userdet_qry);
			if(is_array($get_userdet_res)) {
		...
		function view_CustomerContainerdetails($customerID) {
			...
			global $global_config, $objSmarty;
			$get_userdet_qry = "SELECT * FROM tbl_customer_container where customer_id = '".$customerID."' order by id";
			$get_userdet_res = $this->SelectQryDirect($get_userdet_qry);
	
			$signate_sig = $get_userdet_res[0]['customer_signature'];
			$employ_sig = $get_userdet_res[0]['employee_signature'];
			
			$formedsig_sign = $global_config["site_globalpath"].'uploads/digitalsignatures/'.$signate_sig;
			$formedemp_sign = $global_config["site_globalpath"].'uploads/digitalsignatures/'.$employ_sig;
			
			$get_userdet_res[0]['signed_name'] = $formedsig_sign;
			$get_userdet_res[0]['employe_name'] = $formedemp_sign;
...
exit;*/
	...
	*****************************************************************************************************************/
	...
	function UpdatecustomerDetails($objarray) {
	   ...
	   if($_FILES['addrss_proof']['name']!=''){ 
			...
			$addrss_proof_fileName =  $global_config['site_localpath']."uploads/address_proof/".trim($addrss_proof_fileName);
			 ...
			 $id_expensive_fileName3  = $_POST['addrss_proof11'];
			 @move_uploaded_file($id_expensive_fileName3);
			 $addrss_proof_fileName =  $id_expensive_fileName3;
		  }
		
		 if($_FILES['addrss_id']['name']!=''){ 
			...
			$id_expensive_fileName3  = $_POST['idproof_test'];
			@move_uploaded_file($id_expensive_fileName3);
			$id_proof_fileName =  $id_expensive_fileName3;
		 }
...
exit;*/
       ...
       if(isset($objarray['invoice_address'])){
		...
		if($objarray['id'] > 0) {
			$strWhereClause	= "id = '".$objarray['id']."'";
			$this->common_Update_Set1(tbl_customer, $strUpdateArray, $strWhereClause,0);
			//Redirect($global_config['siteadmin_globalpath'].'customer_list.php');
		}
		
		$Last_updateid = $objarray['id'];
		$get_authuser_qry1 = "SELECT id FROM tbl_customer_container where customer_id='".$Last_updateid."' order by id asc";
		$Final_Auth_user1 = $this->SelectQryDirect($get_authuser_qry1);
	
		if($Last_updateid !='') {
			//$number_of_container = count($objarray['container_name']);
				for($n=0;$n<count($Final_Auth_user1);$n++) {
							...
							$strUpdateArray = array(
									array("customer_id", 	$Last_updateid),
									array("container_type", 	$container_name_val),
									array("container_number", 	$container_number_val),
									array("contract_start_date", 	$contract_start_date),	
									array("contract_next_due_date", $contract_start_date),											
							...
							$strWhereClause	= "id  = '".$Final_Auth_user1[$n]['id']."'";
							$update = $this->common_Update_Set1(tbl_customer_container, $strUpdateArray, $strWhereClause,0);
						}	
					
						$get_authuser_qry2 = "SELECT id FROM tbl_authorised_users where customer_id='".$Last_updateid."' order by id asc";
						$Final_Auth_user2 = $this->SelectQryDirect($get_authuser_qry2);

		 	// $number_of_author = count($objarray['author_user_name']);	
			  for($m=0;$m<count($Final_Auth_user2);$m++) {			
							//author_user_name start
							...
							$strWhereClause	= "id  = '".$Final_Auth_user2[$m]['id']."'";
							$update = $this->common_Update_Set1(tbl_authorised_users, $strUpdateArray, $strWhereClause,0);
						}	
			}
...
/*		echo '<pre>';
	...
	*****************************************************************************************************************/
	...
	function exportCSVPaymetReceipts($objarray) {
		global $objSmarty, $global_config;

		$CheckContainerName = "SELECT a.*, b.first_name, b.sur_name  FROM tbl_payment_history as a JOIN  tbl_customer as b  ON a.customer_id = b.id "; 
		$CheckContainerName_Result = $this->SelectQryDirect($CheckContainerName,0);
	...
	public function sendcsvattach_mail($output) {
		   ...
		   global $objSmarty, $global_config;
			...
			$subject = 'Payment Receipt from Container site!';
			//$to = 'saravanan@securenext.in';
			//$cc = 'dan.sheriff@hotmail.com';
	...
	function exportCSVPaymetEmailsend($objarray) {
...
//		$this->sendcsvattach_mail($output);		
		header('Content-type: application/xls');
		header('Content-Disposition: attachment; filename="Payment_Receipts.xls"');
	    @move_uploaded_file($global_config['site_localpath']."uploads/payment_receipt/".$output);
		echo $output;
		exit;
	}	
	...
	*****************************************************************************************************************/
	...
	function getDeAssignContainerList($PerPage='', $Page='', $Display='',$objArray='') {
		...
		$dataResult = array();
			foreach($rsValues as $key => $dataVal) {
					...
					//$license_fee_amount_due = ( $overdue / 30 ) * $dataVal['licence_amount_vat'];
					/*echo 'overdue is ('. $overdue . '/ 30) * dataval : '. $dataVal['licence_amount_vat'];
					echo '<br>';
					echo ( $overdue / 30 ) * $dataVal['licence_amount_vat'];*/
					...
					if ($license_fee_amount_due_with_decimal == $license_fee_amount_due_with_decimal && $license_fee_amount_due_with_decimal > 0) {
						/*echo $license_fee_amount_due_with_decimal;
						echo '<br>';
					...
					//(License Fee Due / Repayable + Deposit Fee Due / Repayable + Miscellaneous Fee Due / Repayable + Penality Fee Due)
					//$Repayable_Total_amount_due_with_decimal = $dataVal['licence_amount_vat'] - ( $overdue / 30 ) * $dataVal['licence_amount_vat'];	
				...
				/*	$Repayable_Total_amount_due_with_decimal_without_symbol = $license_fee_amount_due_without_symbol_without_symbol + ( - $Res_get_container_price_name[0]['deposit_amount']) + $dataVal['miscellaneous'] + $penality_amount;							
					$Repayable_Total_amount_due_with_decimal = $license_fee_amount_due_with_decimal + ( - $Res_get_container_price_name[0]['deposit_amount']) + $dataVal['miscellaneous'] + $penality_amount;							
					*/
	...
	function getViewPaymentReceiptList($PerPage='', $Page='', $Display='',$objArray='') {
		global $objSmarty, $global_config;
	...
	function getPreviousPaymentHistoryList($PerPage='', $Page='', $Display='',$objArray='') {
		...
		$dataResult = array();
			foreach($rsValues as $key => $dataVal) {
					...
					$yrdata= strtotime($dataVal['contract_start_date']);
					$contract_start_date =  $dataVal['contract_start_date'];
					$contract_next_due_date =  $dataVal['contract_start_date'];
					$Related_priod = date('d/m/Y ', strtotime(str_replace("/", '-', $dataVal['contract_start_date']) . " +30 days"));
					$Related_priod_Result = $dataVal['contract_start_date'] .' - '. $Related_priod;
					$current_date = date("d/m/Y ");// current date
					$diff = abs(strtotime($current_date) - strtotime($dataVal['contract_start_date']));
					...
					$rsValues[$key]['Related_priod_Result'] = $Related_priod_Result;		
					$rsValues[$key]['contract_next_due_date'] = $dataVal['contract_start_date'];
					$overdue =  ceil(abs($current_date-$contract_start_date));
					$startTimeStamp = strtotime("19/07/2016");
					$endTimeStamp = strtotime("09/06/2016");
			...
			foreach($data as $dataVal) {
				$lastrow = '';
					...
					$dataResult[] = array(
						...
						'1'	 => $dataVal['deposit_amount'],
						'2'	 => $dataVal['licence_amount'],	
						'3'	 => $dataVal['vat_amount'],							
						'4'	 => $dataVal['licence_amount_vat'],	
						'5'  => $create_date,
						'6'  => $lastrow
